...
const getAlumno = (request,response) => {
    connection.query("SELECT tbl_alumno.ID_Alumno,tbl_alumno.Nombre, tbl_carrera.Carrera, tbl_alumno.Email FROM tbl_alumno INNER JOIN tbl_carrera WHERE tbl_alumno.FK_Carrera = tbl_carrera.ID_Carrera ",
    (error,results)=>{
        if(error)
        throw error;
